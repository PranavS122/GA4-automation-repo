...
from utils_view import PageSelector, PageSelector_Adobe
import re

class GA4_params_tester_view():

    @staticmethod

    def ga4_params_view(url_dict_list, max_retries=6):
        result_data_list = []
        ...
        for url_dict in url_dict_list:
            for url, inner_dict in url_dict.items():
                ...
                elif re.search(r'eephone', url, re.IGNORECASE):
                ...
                for _, params_inner_dict in params_dict.items():
                    match_count = sum(1 for key, expected_value in inner_dict.items() if
                    key in params_inner_dict and 
                    (str(expected_value).lower() == str(params_inner_dict[key]).lower() if isinstance(expected_value, int) else expected_value.lower()))
...
class Adobe_params_teter_view():

    @ staticmethod

    def view_adobe(url_dict_list, max_retries=6):
        result_data_list = []
        ...
        for url_dict in url_dict_list:
            for url, inner_dict in url_dict.items():
                ...
                if re.search(r'goodyear', url, re.IGNORECASE):
                    script_function = PageSelector_Adobe.selenium_goodyear_script
                else:
                    print(f"No specific script found for {url}. Skipping this URL.")
                    continue
